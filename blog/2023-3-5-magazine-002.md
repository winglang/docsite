...
Here is a summary of the topics in today's issue:
 ...
 - 🔍 [Deep dive: user resources](#user-defined-resources) - Create reusable building blocks organized into logical units that abstract both functional logic and infrastructure
 - 🚀 [New: Introducing duration and datetime types](#introducing-new-types-to-the-language) - our primary engineering focus is to finish implementing the compiler so we can actually start using Wing to build some real stuff
 - 🚀 [New: json type support](#json-type-support) - First class support for JSON in Wing
 - 🚀 [New: copy and copy_mut](#copy--copy_mut) - Convert mutable and immutable types of wing primitive collections
 - 🗣️ [Updates to our contribution guide](#the-contribution-guide) - Easier navigation through the guide and accessibility to  day-to-day workflows
...
We talked about the new map view in the Wing Console, user-defined resources and played some music.
...
For example, we are now working on [User story #15](https://github.com/winglang/wing/blob/ekeren/task-list-with-api-and-db/docs/06-contributors/999-rfcs/000-stories/story-15.md) which includes support for 
...
The [Wing Console](https://docs.winglang.io/getting-started/console) is a desktop application that can be used to interact with Wing applications
...
![console](https://user-images.githubusercontent.com/2212620/224544820-c0ae0f29-1735-498e-a7b2-72c37d33e8d9.png)
...
![logs-filtering](https://user-images.githubusercontent.com/2212620/224544846-6f538897-fcb7-4605-8ad6-5c464ab41845.png)
...
We are still enriching the [Wing Language Specification](https://docs.winglang.io/reference/spec) and increasing the set of features we believe are required to build cloud applications with Wing.
...
Wing is a statically-typed language while JSON is dynamically typed. So we need a way to safely bridge between the worlds. To support this, Wing types will have a `from_json()` static method which will allow move from the unsafe world of JSON to the safe world of static typing. 

Contrary to how JSON works in TypeScript or JavaScript, the `from_json()` methods have built in schema validation, which means that if a call succeeded, developers can be certain that the designated type is correct.
...
Our built-in collections now have `copy` and `copy_mut` functions 💪 ([#1427](https://github.com/winglang/wing/pull/1427)). These functions allow you to easily convert between the mutable and immutable collections. We had an interesting discussion around what to call these functions and decided to align them with our philosophy of *immutable by default*. This means that when you don't specify something, it's by default immutable. Even when you copy a mutable collection using the `copy` function, you will get an immutable version. We believe this is a good start for these functions and look forward to seeing how they will benefit your development projects.
