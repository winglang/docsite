...
Welcome to the **Wing Inflight Magazine**, our periodic update on what's up (yes, two puns) with the
Wing programming language and the community that's starting to gather around the mission to reshape
cloud development.

We will try to publish these frequently to keep them short and sweet, but since this is our first
...
- 🚀 [New: Implicit `await`, explicit `defer`](#implicit-await-and-explicit-defer-ftw) - async calls
  in Wing are implicitly `await`ed. You can use `defer` to get the promise instead.
...
- 🗣️ [RFC: Terraform Resources in Wing](#rfc-using-terraform-resources-in-wing)
...
folks at re:Invent have already heard about the project, and have already formed strong (sometimes
very strong) opinions, and had deep and interesting questions to ask.

This was followed up by a [Hacker News](https://news.ycombinator.com/item?id=34051325) post that
brought about 11K new visitors to [winglang.io](https://winglang.io), with tons of interesting
feedback and relevant conversations:
...
In the next few months, our main focus is implement compiler support for the initial [Wing Language
...
While loops in look like this, where `CONDITION` is any boolean expression. In the spirit of
minimalism, we decided not to require parenthesis around the loop's condition:
...
defined (with `new`) outside the function closure represents an **cloud infrastructure resource**,
...
wire up the bucket information during deployment, it needs to add the right permissions to the
Lambda's IAM policy, it needs to create a code bundle with just the right set of dependencies.
...
Inflight functions are an *isolation boundary* enforced by the compiler. If you try to call
`my_bucket.put()` outside of an `inflight` context the Wing compiler will not let you:
...
library](https://news.ycombinator.com/item?id=34051472). Inflight functions (and resources) are
probably the best example at this point. It is not possible to express this type of isolation and
interaction semantics through a library in existing languages. We've seen some worthy efforts such
as [Pulumi's Function
Serialization](https://www.pulumi.com/docs/intro/concepts/function-serialization/) and [Sam
Goodwin](https://github.com/sam-goodwin)'s [Punchcard](https://github.com/sam-goodwin/punchcard) and
[Functionless](https://functionless.org/) projects. In all of these cases, we think the developer
experience can be further improved when we remove limitations of the host languages and runtimes.

Given their novelty, getting inflight functions right from a syntactic perspective is a top
priority. They need to be intuitive and familiar at the right level. To that end, we recently
decided to [remove the short hand `~>`](https://github.com/winglang/wing/pull/866) to represent
inflight, and instead use a simple `inflight` modifier. We've been debating this for a while, and a
[comment](https://winglang.slack.com/archives/C047QFSUL5R/p1670050350350909?thread_ts=1670030202.531749&cid=C047QFSUL5R)
from a Wingnut, Adam Ruka, tilted the scale in favor of the less quirky syntax (until further
notice).
...
```js
let handler = inflight (e: str): void => {
  // on air
};
```

> (apologies for the endless flight-related puns... It's really unavoidable.)

The next major step for inflights was to establish a composition model that allows inflights to be
composed together, while tracking the operations performed by each one for the purpose of permission
inference and other wiring. We've implemented the first stage in the SDK
...
which talks about declaring a `TaskList` resource in Wing. This is going to be incredible!
...
the call will be awaited, and you can use `defer` to access the promise (and `await` it later).
...
To whet you appetite, here is the list of resources we are considering for our beta. This is just a
...
Here's a [short video](https://www.youtube.com/watch?v=hFIv7YZ-ROU) that shows this new feature.
...
We promise to try and publish these more occasionally, so it's easier to keep track. 
