...
We were also ecstatic to see some initial code contributions to the project: [Owen
Watkins](https://github.com/ogre14t) submitted a PR to [add line
numbering](https://github.com/winglang/wing/pull/928) to the playground.
...
This was followed up by someone posting about Wing on [Hacker
News](https://news.ycombinator.com/item?id=34051325), which brought about 11K new visitors to
[winglang.io](https://winglang.io), with tons of interesting feedback and relevant conversations:
...
Go, *[Wingnuts](https://youtu.be/XrWwsd0F9yk)* ❤️ 
...
let handler = inflight () => {
  ...
  // this is just hypothetical syntax for something like `Promise.all()`
  // see https://github.com/winglang/wing/issues/1004
...
new cloud.Queue().on_message((filename: str): str => {
  // download the file from the bucket
  let data = bucket.get(filename);

  // clone the git repository to a temp directory
  let git = new git.Repository(fs.mkdtemp());
  git.clone(repo.repository_url, depth: 0);

  // update the file
  fs.File.write(fs.Path.join(git.directory, filename), data))

  // create a commit
  git.add(filename)
  git.commit(message: "update file.txt");
