...
We are back with another issue of the **Wing Inflight Magazine** and excited to share some updates about [Winglang](https://winglang.io), **an open source-programming language for the ☁️ cloud**.
...
It is designed to be a **familiar and friendly** language to learn for developers who come from modern object-oriented background, so it will take you [5 minutes](https://github.com/winglang/workshop-react/blob/main/cheatsheet.md) to learn.
...
- 🛟 [The bang `!` operator](#)
- 🏋️ [Ability to lift collection of inflight closures](#)
- 🗄️ [`fs.appendFile()`](#)
...
> I spent like two hours trying to figure out the right emojis here so please take a minute to appreciate them :-)
### Deploying AWS resources and APIs inside a private VPCs

![[2024-03-13-private-api.png]]

Users tell us they are looking for ways to use Wing to deploy API-based services inside their VPCs. We've spent some time learning about [the challenges people have](https://www.winglang.io/blog/2024/02/15/private-api-gateways) and figured we can offer a awesome experience in Wing for this use case.

We published this [blog post](https://www.winglang.io/blog/2024/02/09/private-api-gateway-aws) and described how to configure the `tf-aws` platform to deploy your application's AWS resources and APIs inside a VPC, with support for creating a whole new VPC or deploying into a VPC that's already setup in your environment.
...
![[2024-03-13-console-new-look.mp4]]
...
We are continuing to add language tools for safely dealing with `nil`s.

The bang operator (`!`) performs a *runtime assertion* that an optional has a value (is not `nil`), and returns the value.

An error will be thrown at runtime if the value is `nil`. Unlike TypeScript, where `!` is only a type-checking hint, in Wing the bang operator will actually throw an error if there is no value. This avoids propagating `nil` and avoids the common issue of a confusing `"Cannot read properties of undefined"` errors cascading through your program.
...
It is now possible for platform extensions to read parameters from a configuration file (`wing.toml`) or from CLI switches.
