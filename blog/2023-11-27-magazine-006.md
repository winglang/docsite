...
Today, Wing is shipped with the following built-in platforms:
 - `sim` - runs your entire app on a single machine. Used for development (using the Wing Console) or for unit tests (using the `test` directive).
...
We decided to change the name of class constructors to `new()` instead of `init()`:
