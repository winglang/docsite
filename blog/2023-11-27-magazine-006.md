...
- 🔥 [Constructors are now called `new()` instead of `init()`](constructors-ar[…]-instead-of-init)
- 🔥 [`http.fetch()` response `body` is not optional anymore](#httpfetch-respo[…]optional-anymore)
